...
DEVICE="/dev/mapper/luks-1c78fbdd-8c3d-4584-b6c6-baadf470d0a3"
...
blktrace "$DEVICE" -a read -o - | blkparse -f "MYSTATS %12S %12n %12N \n" -q -i  - | uniq | grep -E "^MYSTATS" | grep -E "^.* [0-9]?[0-9]?[0-9]?[0-9]?[0-9]?[0-9] $"  | sed -r 's~MYSTATS +([01-9]+) +([01-9]+) +[01-9]+~dd if='"$DEVICE"' of=/dev/null bs=512 count=\2 skip=\1~' >> "$tmpdir/blktrace_home.txt"
